...
func (sc *SimpleContract) Create(ctx api.TransactionContextInterface, key string) error {
...
func (sc *SimpleContract) Update(ctx api.TransactionContextInterface, key string, value string) error {
	existing, err := ctx.GetStub().GetState(key)
...
func (sc *SimpleContract) Read(ctx api.TransactionContextInterface, key string) (string, error) {
	existing, err := ctx.GetStub().GetState(key)
